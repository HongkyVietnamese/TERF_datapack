...
$clone ~ ~ ~ ~$(size_x) ~$(size_y) ~$(size_z) to terf:intermediary_technical_storage_dimension -30000000 0 -30000000 replace
#replace all blocks in the storage dimension with #terf:unwarpable using fill, basically counting how many blocks are not #terf:unwarpable and count ship blocks
...
$execute in terf:intermediary_technical_storage_dimension positioned -30000000 0 -30000000 store result score @s terf_data_D run fill ~ ~ ~ ~$(size_x) ~$(size_y) ~$(size_z) air replace
scoreboard players operation @s terf_data_D += @s terf_data_C
